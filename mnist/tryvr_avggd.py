...
import pickle
results = {'train_loss':[],'train_prec':[],'train_sqgd':[],
            'test_loss':[],'test_prec':[]}
import math
...
#    flat_grad = gather_flat_grad(params)/len(train_loader.dataset)

    # calculate fullgradient
    optimizer.zero_grad()
    ...
    optimizer.setfullgrad(len(train_loader.dataset))
    optimizer.settmpx()
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if gradient is None:
            gradient = gather_flat_grad(params)*len(data)
        else:
            gradient += gather_flat_grad(params)*len(data)
    ...
    gradient /= len(train_loader.dataset)
    ...
    results['train_sqgd'].append(sqgd)
...
def main():
    ...
    #svrg = optim.SVRG(sgd)
    adasamvr = optim.AdaSAMVR(sgd,period=1,hist_length=20,damp=1e-4,gamma=0.9,precision=1)
