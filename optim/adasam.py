...
class AdaSAM(Optimizer):
    ...
    def __init__(self, optimizer=None,alpha=1.0,beta=1.0,
        ...
        if len(self.param_groups) != 1:
            raise ValueError("AdaSAM doesn't support per-parameter options "
                             "(parameter groups)")
    ...
    def __setstate__(self, state):
        super(AdaSAM, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        if cnt>0 and cnt % period == 0:
            ...
            Gammak = (pinv(((Rk.t()@Rk)+delta*(Xk.t()@Xk)).double())).to(self.dtype)@(Rk.t()@res)
            tmp_matrix = alpha*Xk
            tmp_matrix.add_(alpha*beta,Rk)
            x_delta = beta*res - tmp_matrix@Gammak
            #x_delta = beta*res - (alpha*Xk+alpha*beta*Rk) @ (pinv(Rk.t()@Rk+delta*Xk.t()@Xk)@(Rk.t()@res))
            tmp = x_delta.dot(res)      # simplified check
            if tmp <=0:
